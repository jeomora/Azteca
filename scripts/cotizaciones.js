...
function setAdminTable(){
	...
	$("html").block({
		centerY: 0,
		message: '<div style="width:80rem;height:50rem;background-image:url(assets/img/clock.gif)"><h1 style="margin:0;padding-top:5rem">Cargando cotizaciones</h1><h4 id="gotam"></h4></div>',
	...
	getAdminTable()
		.done(function (resp) {
			var tamanio = resp.cotizaciones.length;
			$("#gotam").text("0 / "+tamanio);
			$.each(resp.cotizaciones, function(indx, value){
				$("#gotam").text(indx+" / "+tamanio);
				appending(value);
			});	
