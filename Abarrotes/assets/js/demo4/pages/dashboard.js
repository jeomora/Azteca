...
$(function($) {
    $.each(tiendas, function(indx, value){
        getPeds(value).done(function(resp){
            if (resp.cuantas == null) {
                $("#lun"+indx).html("<button type='button' class='btn btn-bold btn-label-brand btn-sm' data-toggle='modal' data-target='#kt_modal_1' "+
                    "id='luninfo' data-id-cot='"+value+"'>0 de "+resp.noprod.noprod+"</button>");
            } else {
                $("#lun"+indx).html("button type='button' class='btn btn-bold btn-label-brand btn-sm' data-toggle='modal' data-target='#kt_modal_1' "+
                    "id='luninfo' data-id-cot='"+value+"'>"+resp.cuantas.cuantas+" de "+resp.noprod.noprod+"</button>");
            }
            $("#gen"+indx).html("<button type='button' class='btn btn-bold btn-label-brand btn-sm' data-toggle='modal' data-target='#kt_modal_1' "+
                    "id='allinfo' data-id-cot='"+value+"'>"+resp.allcuantas.cuantas+" de "+resp.noall.total+"</button>");
            $("#vol"+indx).html("<button type='button' class='btn btn-bold btn-label-brand btn-sm' data-toggle='modal' data-target='#kt_modal_1' "+
                    "id='volinfo' data-id-cot='"+value+"'>"+resp.volcuantas.cuantas+" de "+resp.novol.total+"</button>");
        })
    });
...
$(document).off("click", "#allinfo").on("click", "#allinfo", function(event) {
    ...
    getPedidos($(this).data("idCot")).done(function (resp){
        if (resp) {
            $("#existenciasnot").html("");
            $.each(resp.existenciasnot,function(index, value){
                $("#existenciasnot").append("<tr><td>"+(index+1)+"</td><td>"+value.codigo+"</td><td>"+value.descripcion+"</td></tr>")
            })
        }
...
function getPedidos(tienda){
    return $.ajax({
        url: site_url+"/Pedidos/getPedidos/"+tienda,
        type: "POST",
        dataType: "JSON",
    });
}
...
var KTDashboard = function() {
    ...
    //  Tipos de usuarios
    var _initSparklineChart2 = function(src, data, color, border) {
        if (src.length == 0) {
            return;
        }
        ...
        var config = {
            type: 'line',
            data: {
                labels: ["Proveedores", "Personal", "Sucursales"],
                datasets: [{
                    label: "",
                    borderColor: color,
                    borderWidth: border,
    ...
    var quickStats = function() {
        getMeses().done(function (resp) {
            var meses = [];
            $.each(resp,function(index,val){
                meses.push(val.cuenta);
            })
            _initSparklineChart($('#kt_chart_quick_stats_1'),meses , KTApp.getStateColor('brand'), 3);
        })
        getMesesProv().done(function (resp2) {
            var meses2 = [];
            $.each(resp2,function(index,val2){
                meses2.push(val2.cuanto);
            })
            _initSparklineChart2($('#kt_chart_quick_stats_3'),meses2 , KTApp.getStateColor('success'), 3);
        })
    ...
    var getCambio = function() {
        getCambios().done(function (resp) {
            var meses = "";var cual = "";var archivo = "";
            $(".kt-timeline-v2__items").html("")
            $.each(resp,function(index,val){
                console.log("ah perro "+val.id_grupo+" cual "+cual)
                if(val.id_grupo === 2 || val.id_grupo === "2"){
                    cual = "kt-font-success";
                }else if(val.id_grupo === 3 || val.id_grupo === "3"){
                        cual = "kt-font-brand";
                }else if(val.id_grupo === 4 || val.id_grupo === "4"){
                            cual = "kt-font-danger";
                }else{
                    cual = "";
                }
                if (val.accion.toUpperCase() === "SUBE ARCHIVO" || val.accion.toUpperCase() === "SUBE PEDIDOS" || val.accion === "Sube existencias y pedidos" 
                    || val.accion === "Sube existencias y pedidos formato LUNES" || val.accion.toUpperCase() === "SUBE PEDIDOS PENDIENTES") {
                        archivo = '<a href="'+val.despues+'" class="kt-link kt-link--warning kt-font-bolder">Descargar Archivo</a>';
                }else{
                    archivo = ""
                }
                meses = meses + '<div class="kt-timeline-v2__item"><span class="kt-timeline-v2__item-time '+cual+'">'+val.hor+' <br>'+val.fec+'</span>'+
                        '<div class="kt-timeline-v2__item-cricle"><i class="fa fa-genderless '+cual+'"></i></div><div class="kt-timeline-v2__item-text kt-padding-top-5"'+
                        '><span class="kt-timeline-v2__item-text--bold '+cual+'">'+val.nombre+'</span><span class="kt-timeline-v2__item-text--bold"> ('+val.accion+
                        ')</span><br>'+val.antes+'<br>'+archivo+'</div></div>'
                
                
            })
            $(".kt-timeline-v2__items").html(meses)
        })
    }
...
function getMeses() {
    return $.ajax({
        url: site_url+"/Inicio/getMeses",
        type: "POST",
        dataType: "JSON"
    });
}

function getMesesProv() {
    return $.ajax({
        url: site_url+"/Inicio/getMesesProv",
        type: "POST",
        dataType: "JSON"
    });
}
...
function getCambios() {
    return $.ajax({
        url: site_url+"/Inicio/getCambios",
        type: "POST",
        dataType: "JSON"
    });
}