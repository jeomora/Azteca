...
$(function($) {
    $.each(tiendas, function(indx, value){
        getPeds(value).done(function(resp){
            ...
            $("#far"+indx).html("<button type='button' class='btn btn-bold btn-label-success btn-sm' data-toggle='modal' data-target='#kt_modal_6' "+
                    "id='allinfo2' data-id-cot='"+value+"'>"+resp.allcuantas2.cuantas+" de "+resp.noall2.total+"</button>");
        })
    });
});
...
$(document).off("click", "#allinfo2").on("click", "#allinfo2", function(event) {
    event.preventDefault();
    getPedidos2($(this).data("idCot")).done(function (resp){
        if (resp) {
            $("#existenciasnot2").html("");
            $.each(resp.existenciasnot,function(index, value){                
                $("#existenciasnot2").append("<tr><td>"+(index+1)+"</td><td>"+value.codigo+"</td><td>"+value.descripcion+"</td></tr>")
            })
        }      
    });
});
...
function getPedidos(tienda){
    return $.ajax({
        url: site_url+"/Pedidos/getPedidee/"+tienda,
        type: "POST",
        dataType: "JSON",
    });
}

function getPedidos2(tienda){
    return $.ajax({
        url: site_url+"/Pedidos/getPedidee2/"+tienda,
        type: "POST",
        dataType: "JSON",
    });
}
