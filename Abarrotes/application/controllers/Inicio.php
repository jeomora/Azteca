...
class Inicio extends MY_Controller {
	...
	function __construct(){
		...
		$this->load->model("Productos2_model", "pr2_md");
		$this->load->model("Cotizaciones_model", "cot_md");
		$this->load->model("CotizacionesF_model", "cotf_md");
		...
		$this->load->model("Existencias_model", "ex_md");
		$this->load->model("Existencias2_model", "ex2_md");
	}
	...
	public function index(){
		if($this->session->userdata("username")){
			...
			$data["proveedores"]=$this->user_md->getUsuarios(['usuarios.id_grupo<>'=>1,'usuarios.estatus<>'=>0]);//Son proveedores
			$data["productos"]=$this->pr_md->get(NULL,["estatus <>"=>0]);
			$data["familias"]=$this->fam_md->get();
			$data["cotizados"] = $this->user_md->getCotizadillos2();
			$data["lastCots"] = $this->user_md->getCotizadillos();
			$data["cotizados2"] = $this->user_md->getCotizadillos4();
			$data["lastCots2"] = $this->user_md->getCotizadillos3();
			$data['links'] = ['/assets/css/waves'];
			$data["cotizad"] = $this->pr_md->getCotizadillos();
			$data["cotizad2"] = $this->pr_md->getCotizadillos2();
			$data["cotizad3"] = $this->pr_md->getCotizadillos3();
			$data["cotizad4"] = $this->pr_md->getCotizadillos4();

			
			if ($user["id_grupo"] === "4" || $user["id_grupo"] === 4) {
				$data['scripts'] = ['/scripts/dashboard'];
				$this->estructura("Admin/welcome", $data);
			}elseif ($user["id_grupo"] === "1" || $user["id_grupo"] === 1) {
				$data['scripts'] = ['/scripts/dashboard'];
				$this->estructura("Admin/welcome", $data);
			}elseif ($user["id_grupo"] === "2" || $user["id_grupo"] === 2) {
				$data["cots"] = $this->cot_md->get(NULL,[ "estatus"=>1,"id_proveedor"=>$user["id_usuario"] ]);
				$data["cotsf"] = $this->cotf_md->get(NULL,[ "estatus"=>1,"id_proveedor"=>$user["id_usuario"] ]);
				$data['links'] = ['/assets/css/pestania'];
				$data['scripts'] = ['/scripts/proveedores','/scripts/pestania'];
				$data["user"] = $user;
				$this->estructura("Proveedores/cotizaciones", $data);
			}elseif ($user["id_grupo"] === "3" || $user["id_grupo"] === 3) {
				$data["noall"] = $this->pr_md->getAllCount(NULL)[0];
				$data["noall2"] = $this->pr2_md->getAllCount(NULL)[0];
				$data["allcuantas"] = $this->ex_md->getAllTienda(NULL,$user["id_usuario"])[0];
				$data["allcuantas2"] = $this->ex2_md->getAllTienda(NULL,$user["id_usuario"])[0];
				...
				$data['links'] = ['/assets/css/folder'];
				$data['scripts'] = ['/scripts/tiendas','/scripts/folder'];
				$data["user"] = $user;
				$this->estructura("Tiendas/existencias", $data);
			}
		}else{
