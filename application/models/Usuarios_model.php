...
class Usuarios_model extends MY_Model {
	...
	public function getDudes($where=[]){
		...
		->where("u.id_grupo", 2)
		->where("u.estatus <>", 0)
		->where("u.id_usuario IN (u.id_usuario IN (select c.id_proveedor from cotizaciones c where WEEKOFYEAR(c.fecha_registro) = WEEKOFYEAR(CURDATE()) GROUP BY c.id_proveedor)")
	...
	public function getH($where=[]){
		$this->db->select("*")
		->from($this->TABLE_NAME)
		->where("(id_usuario = 6 OR id_usuario = 17 OR id_usuario = 42)")
		->order_by("id_usuario","ASC");
		if ($where !== NULL) {
			if (is_array($where)) {
				foreach ($where as $field=>$value) {
