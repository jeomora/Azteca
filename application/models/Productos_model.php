...
class Productos_model extends MY_Model {
	...
	public function getProdFam($where = [],$prove){
		...
		$this->db->select("c.observaciones,ff.id_familia,f.id_producto AS sem1,f2.id_producto AS sem2, c.id_cotizacion,c.id_proveedor,c.id_producto,c.precio,c2.precio_promocion precio2,conv.conversion, 
			...
			WEEKOFYEAR(DATE_SUB(CURDATE(),INTERVAL 7 DAY)) AND c.id_proveedor = ".$prove." LEFT JOIN conversiones conv ON p.id_producto = conv.id_producto AND conv.id_proveedor = ".$prove." WHERE p.estatus <> 0");
		if ($where !== NULL){
			if(is_array($where)){
				foreach($where as $field=>$value){
					if ($value !== NULL) {
