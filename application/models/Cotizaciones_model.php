...
LEFT JOIN usuarios u ON c.id_proveedor = u.id_usuario WHERE WEEKOFYEAR(c.fecha_registro) = 31 AND 
(p.precio_sistema - c.precio_promocion) > (p.precio_sistema * 0.2) OR 
WEEKOFYEAR(c.fecha_registro) = 31 AND (c.precio_promocion - p.precio_sistema) > (p.precio_sistema * 0.2) ORDER BY diferencia DESC")->result();
		if ($result) {
			if (is_array($where)) {
				return $result;
			} else {
