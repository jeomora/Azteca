...
class Cotizaciones extends MY_Controller {
	...
	public function fill_formato1(){
		...
		if (($id_proves <> "3" || $id_proves <> 3) && ($id_proves <> "6" || $id_proves <> 6)) {
			...
			$hoja1->getColumnDimension('F')->setWidth("50");
			...
			if ($array){
				...
				$cotizacionesProveedor = $this->ct_mdl->getPedidosAll($where, $fecha->format('Y-m-d H:i:s'), 0);
					...
					if ($cotizacionesProveedor){
						...
						if($i > 0){
							$flagBorder = $flag1 ;
							$this->excelfile->getActiveSheet()->getStyle('A'.$flagBorder1.':F'.$flagBorder)->applyFromArray($styleArray);
							$flagBorder1 = $flag1;
						}
							...
							if ($value['articulos']) {
								foreach ($value['articulos'] as $key => $row){
									...
									if ($id_proves === "VOLUMEN" || $id_proves === "AMARILLOS") {
										...
										$hoja->setCellValue("CC{$flag}", "=N".$flag."+X".$flag."+AB".$flag."+AF".$flag."+AJ".$flag."+AN".$flag."+AR".$flag."+AV".$flag."+BZ".$flag."");
										//End: TOTALES PEDIDOS PENDIENTES
									}else{
	...
	private function fill_duerazo(){
		...
		foreach ($array as $key => $v3) {
			...
			$hoja1->setCellValue("F".$flag2."", "PROMOCIÓN");
