...
class Cotizaciones extends MY_Controller {
				...
				$cotizacionesProveedor = $this->ct_mdl->getPedidosAll($where, $fecha->format('Y-m-d H:i:s'), 0);
					...
					if ($cotizacionesProveedor){
						...
						if ($id_proves === "VOLUMEN" || $id_proves === "AMARILLOS"){
							...
							$hoja->setCellValue("R".$flag, "SUMA CEDIS/SUPER");
							...
							$hoja->setCellValue("Y".$flag, "PEDREGAL");
							...
							$hoja->setCellValue("Z".$flag, "PEDREGAL");
	...
	public function comparaExpo(){
		...
		$hoja->setCellValue("S1", "CEDIS/SUPER");
		...
		$hoja->setCellValue("Y1", "SUMA CEDIS/SUPER");
		...
		$hoja->setCellValue("AE1", "PEDREGAL");
	...
	private function fill_duerazo(){
		...
		$hoja->setCellValue("P".$flag, "SUPER INDUSTRIAL");
		$this->cellStyle("S".$flag, "C2B90A", "000000", TRUE, 12, "Franklin Gothic Book");
		$hoja->setCellValue("S".$flag, "SUMA CEDIS/SUPER");
		...
		$hoja->setCellValue("AA".$flag, "PEDREGAL");
			...
			foreach ($cotizacionesProveedor as $key => $value) {
				...
				if ($value['articulos']) {
					foreach ($value['articulos'] as $key => $row){
						...
						$hoja->setCellValue("S{$flag}", "=K".$flag."+BA".$flag);
						$hoja->setCellValue("T{$flag}", "=L".$flag."+BB".$flag);
						$hoja->setCellValue("U{$flag}", "=O".$flag."+BC".$flag);
	...
	private function fill_hermanos(){
		...
		$this->excelfile->getActiveSheet()->getStyle('BD'.$flag)->applyFromArray($styleArray);
		$fecha = new DateTime(date('Y-m-d H:i:s'));
		$where=["ctz_first.id_proveedor"=>6,"prod.estatus" => 1];//Semana actual
		$intervalo = new DateInterval('P2D');
		$fecha->add($intervalo);
		$cotizacionesProveedor = $this->ct_mdl->getPedidosAll($where, $fecha->format('Y-m-d H:i:s'), 0);
		$flag1 = 5;
		...
		$meses = array("ENERO","FEBRERO","MARZO","ABRIL","MAYO","JUNIO","JULIO","AGOSTO","SEPTIEMBRE","OCTUBRE","NOVIEMBRE","DICIEMBRE");
		$fecha =  $dias[date('w')]." ".date('d')." DE ".$meses[date('n')-1]. " DEL ".date('Y') ;
		$file_name = "FORMATO 19 HNOS & ORSA ".$fecha.".xlsx"; //Nombre del documento con extención
		$excel_Writer = PHPExcel_IOFactory::createWriter($this->excelfile, "Excel2007");
		header("Content-Type: application/vnd.ms-excel; charset=utf-8");
		header("Content-Disposition: attachment;filename=".$file_name);
		header("Cache-Control: max-age=0");
