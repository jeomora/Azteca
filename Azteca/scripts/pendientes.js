...
function uploadPendientes(formData) {
	return $.ajax({
		url: site_url+"Pedidos/upload_pendientes/",
		type: "POST",
		cache: false,
		contentType: false,
		processData:false,
...
function getPendientes() {
	return $.ajax({
		url: site_url+"/Pedidos/getPendientes/",
		type: "POST",
		dataType: "JSON"
	});
}
	...
	.done(function (resp) {
		...
		if(resp.pendientes){
			$.each(resp.pendientes, function(indx, value){
				$(".tablePendv").append('<tr><td>'+value.codigo+'</td><td>'+value.nombre+'</td><td>'+value.cedis +'</td><td>'+value.abarrotes+
