...
class Exislunes_model extends MY_Model {
	...
	public function getLunExist($where=[],$tienda){
		$this->db->select("ex.id_existencia,p.codigo,p.descripcion,ex.cajas,ex.piezas,ex.pedido FROM ex_lunes ex LEFT JOIN pro_lunes p on ex.id_producto = p.codigo WHERE (WEEKOFYEAR(ex.fecha_registro) = WEEKOFYEAR(CURDATE()) AND YEAR(ex.fecha_registro) = YEAR(CURDATE())) and ex.id_tienda = ".$tienda." and p.estatus = 1");
	...
	public function getLunExistNot($where=[],$tienda){
		$this->db->select("pp.codigo,pp.descripcion from pro_lunes pp WHERE codigo NOT IN (SELECT p.codigo FROM ex_lunes ex LEFT JOIN pro_lunes p on ex.id_producto = p.codigo WHERE (WEEKOFYEAR(ex.fecha_registro) = WEEKOFYEAR(CURDATE()) AND YEAR(ex.fecha_registro) = YEAR(CURDATE())) and ex.id_tienda = ".$tienda.") and pp.estatus = 1 ");
